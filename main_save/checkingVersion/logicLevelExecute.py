...
def niftyFifty():
    ...
    # df.to_csv("/root/liveAlgos/HzVz/checkingVersion/one.csv")
    ...
    return AlgoStartDate.date(), AlgoStartDateNiftyClose, quantityNifty, niftyMTM, netProfit, niftyMtmPercentage, niftynetProfitPercentage
    ...
    def run_strategy(self, portfolio):
        try:
            ...
            while True:
                ...
                if (currentHour != lastPrintHour) or (algoStart == True):
                    ...
                    try:
                        ...
                        AlgoStartDate, AlgoStartDateNiftyClose, quantityNifty, niftyMTM, netProfit, niftyMtmPercentage, niftynetProfitPercentage = niftyFifty()
                        ...
                        infoMessage(algoName=algoName,message=(f"START DATE: {AlgoStartDate} | START PRICE: {round(AlgoStartDateNiftyClose, 2)} | {quantityNifty}"))
                        infoMessage(algoName=algoName,message=(f"NIFTY MTM: {round(niftyMTM)}[{round(niftyMtmPercentage, 1)}%] | "f"NIFTY NET P/L: {round(netProfit)}[{round(niftynetProfitPercentage, 1)}%]"))
            ...
            for stock in subPortfolio:
                ...
                try:
                            ...
                            if currentTime >= time(15, 15) and currentTime < time(15, 18):
                                ...
                                if self.breakEven[stock] and currentPrice > row['EntryPrice']:
                                    if rsi_current < int(config["inputParameters"]["EntryTimeRsi"]):
                                        ...
                                        strategyLogger.info(f"BreakevenExit: Rsi:{rsi_current}, {stock}, {currentDatetime}")

                                    elif rsi_current > int(config["inputParameters"]["EntryTimeRsi"]):
                                        self.breakEven[stock] = False
                                        writeJson(f"breakEven{stock}", self.breakEven[stock])
                                        infoMessage(algoName=algoName, message=f"Position_continue {stock}")
                                        strategyLogger.info(f"Position_continue: Rsi:{rsi_current}, {stock}")
                    ...
                    if self.stockDict[stock].openPnl.empty and nowTotalTrades < TotalTradeCanCome:
                        ...
                        entry_condition_1 = (time(15, 15) <= currentTime < time(15, 17)) and (rsi_current > int(config["inputParameters"]["EntryTimeRsi"])) and (rsi_current > rsi_previous)
